...
import { Visitenkarte_Standard } from "../../../../api/Api"
...
export default class Product extends Component {
  ...
  getEndFormat = async () => {
    ...
    try {
      const { data } = await Visitenkarte_Standard.getAllEndFormat()
      let newData = []
  ...
  getMaterial = async () => {
    ...
    this.debounceTimeout = setTimeout(async () => {
      try {
        ...
        const { data } = await Visitenkarte_Standard.digiPrintMaterial(this.state.initialValues)
        let newData = []
        data.data.forEach((element) => {
          newData = [
            ...newData,
  ...
  handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await Visitenkarte_Standard.record(this.state.initialValues, this.loaderPercentage)
