...
export default class Product extends Component {
  ...
  debounceTimeout
  getMaterial = async () => {
    const { endFormat, endFormatId, count } = this.state
    clearTimeout(this.debounceTimeout)
    this.debounceTimeout = setTimeout(async () => {
      try {
        this.setState({ material: [], materialShow: false })
        const selectedEndFormat = endFormat.find((x) => x.value === endFormatId)
        const sendData = { height: selectedEndFormat.height, width: selectedEndFormat.width, count: count }

        const { data } = await RollUp.digiPrintMaterial(sendData)
        let newData = []
        ...
        const materialSelected = data.data.filter((x) => x.key === "Display Film 190µ")

        await this.setState({
          material: newData,
          materialId: materialSelected && materialSelected[0] && materialSelected[0].id,
          materialShow: true
        })
      } catch (error) {
        console.error(error)
  ...
  handleSubmit = async () => {
    const { endFormat, endFormatId, count, materialId } = this.state
    ...
    try {
      ...
      const selectedEndFormat = endFormat.find((x) => x.value === endFormatId)
      const sendData = {
        height: selectedEndFormat.height,
        width: selectedEndFormat.width,
        count: count,
      ...
      const { data } = await RollUp.record(sendData, this.loaderPercentage)
  ...
  render() {
    ...
    const { endFormat, steps, material, materialShow, count, percent, show, endFormatId, materialId, showLogin, loadingPage } = this.state
    return (
      <>
