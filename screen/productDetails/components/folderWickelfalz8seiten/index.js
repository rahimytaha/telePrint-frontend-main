...
import { FolderWickelfalz8seiten } from "../../../../api/Api"
...
export default class Product extends Component {
  ...
  getEndFormat = async () => {
    ...
    try {
      const { data } = await FolderWickelfalz8seiten.getAllEndFormat()
      let newData = []
  ...
  getMaterial = async () => {
    ...
    this.debounceTimeout = setTimeout(async () => {
      try {
        ...
        const { data } = await FolderWickelfalz8seiten.digiPrintMaterial(this.state.initialValues)
        let newData = []
        data.data.forEach((element) => {
          newData = [
            ...newData,
  ...
  handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await FolderWickelfalz8seiten.record(this.state.initialValues, this.loaderPercentage)
      ...
      <>
        ...
        <div className="product-order">
          ...
          <div className="product__button flex-j-s">
            {steps === "step1" ? (
              <Button onClick={() => this.handleStep(2)} disabled={loadingPage || !materialShow} appearance="primary">
                nächste Stufe
              </Button>
            ) : percent > 0 ? (
