...
export default class Product extends Component {
  ...
  getEndFormat = async () => {
    ...
    try {
      const { data } = await folder.getAllEndFormat()
      let newData = []
  ...
  getMaterial = async () => {
    ...
    this.debounceTimeout = setTimeout(async () => {
      try {
        ...
        const { data } = await folder.digiPrintMaterial(this.state.initialValues)
        let newData = []
        data.data.forEach((element) => {
          newData = [
            ...newData,
  ...
  handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await folder.record(this.state.initialValues, this.loaderPercentage)
