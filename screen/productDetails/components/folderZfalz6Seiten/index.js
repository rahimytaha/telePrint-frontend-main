...
import { FolderZfalz6Seiten } from "../../../../api/Api"
...
export default class Product extends Component {
  ...
  getEndFormat = async () => {
    ...
    try {
      const { data } = await FolderZfalz6Seiten.getAllEndFormat()
      let newData = []
  ...
  getMaterial = async () => {
    ...
    this.debounceTimeout = setTimeout(async () => {
      try {
        ...
        const { data } = await FolderZfalz6Seiten.digiPrintMaterial(this.state.initialValues)
        let newData = []
        data.data.forEach((element) => {
          newData = [
            ...newData,
  ...
  handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await FolderZfalz6Seiten.record(this.state.initialValues, this.loaderPercentage)
