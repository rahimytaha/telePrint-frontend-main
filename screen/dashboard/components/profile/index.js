...
const model_privateCustomer = Schema.Model({
  email: StringType().isEmail("Bitte geben Sie eine gültige E-Mail-Adresse ein").isRequired("This field is required."),
...
export default class gride extends Component {
  ...
  //     this.open("success", "Erfolgreich", "data changed");
  //   } catch (error) {
  //     console.log(error);
  //   }
  // };
  ...
  handleSubmit = async () => {
    try {
      ...
      const { data, status } = await account.update(this.state.initialValue)
      if (status === 200) {
        // const cookies = new Cookies();
        // cookies.set("token", data.data.token);
        // cookies.set("personalData", JSON.stringify(data.data));
        const dataa = await account.getMe()
        if (status === 200) {
          this.setState({ initialValue: { ...dataa.data.data } })
        }

        // localStorage.setItem("token", data.data.token);
        localStorage.setItem("personalData", JSON.stringify(data.data))
        this.open("success", "Erfolgreich", "Ihre Informationen wurden bearbeitet.")
        // Router.push("/");
  ...
  Adressdaten = () => {
    const { initialValue, gender, country, industry, showEdit } = this.state
          ...
          >
            ...
            <div className="flex-j-c w-100">
              <div className="login-input flex">
                ...
                <div className="input-text input-profile">
                  <FormGroup className="input-text-form-wrapper">
                    <FormControl
                      searchable={false}
                      disabled={!showEdit}
            ...
            <div className="main2-dashB-dad w-90">
              ...
              <main className="w-100 min-height-60">
                ...
                <div className="anfrage-form rs-form-control-wrapper">
                  <div style={{ marginBottom: "10px", color: "#E6007E" }} className="main2-anfrage-txt1">
                    Email : {this.state.initialValue?.email}
                  </div>
                  <div style={{ marginBottom: "10px", color: "#E6007E" }} className="main2-anfrage-txt1">
                    Telefonnummer : {this.state.initialValue?.phoneNumber}
                  </div>
                  <Form
                    onCheck={(formError) => {
                      this.setState({ formError })
